...
# トスのスロット別割合
def tmakeplot1(df,df_Team1,Team1,set,number):
  ax = plt.axes()
  ax.set_facecolor("navajowhite")
  df_plot = df_td1(df,df_Team1,set,number,"")
  plt.scatter(df_plot["X"],df_plot["Y"],s = df_plot["COUNT"]*300,color="blue")
  df_plot = df_td1(df,df_Team1,set,number,"p")
  plt.scatter(df_plot["X"],df_plot["Y"],s = df_plot["COUNT"]*300,color="orangered")
  plt.title(f"{Team1} \r\n {set} Setting Distribution    No.:{number}\r\n ALL   Left:{slot1(df,df_Team1,set,number,1,'')}%    Center:{slot1(df,df_Team1,set,number,2,'')}%    Right:{slot1(df,df_Team1,set,number,3,'')}% \r\n POINT   Left:{slot1(df,df_Team1,set,number,1,'p')}%    Center:{slot1(df,df_Team1,set,number,2,'p')}%    Right:{slot1(df,df_Team1,set,number,3,'p')}%")
...
def df_td1(df,df_Team1,set,number,result):
  tdata01 = []
  ...
  for slot in slots:
    ...
    tdata = {
      "X":tzone1(slot,"x"),
      "Y":tzone1(slot,"y"),
      "COUNT":len(df_data1(df,df_Team1,set,slot,number,"a",result)),
    }
...
# レフト・センター・ライトの割合を表示
def slot1(df,df_Team1,set,number,Side,result):
  if result =='':
    if Side == 1:
      slots = ["51","53"]
    elif Side == 2:
    ...
    y = 0
    for slot in slots:
      x = len(df_data1(df,df_Team1,set,slot,number,"a",""))
      y = y + x
  else:
    if Side == 1:
      slots = ["51","53"]
    ...
    y = 0
    for slot in slots:
      x = len(df_data1(df,df_Team1,set,slot,number,"a","p"))
      y = y + x
  return round(y/(len(df_data1(df,df_Team1,set,"",number,"a","")))*100,1)

# データのプロット
def makeplot1(df,df_Team1,Team1,set,slot,number,action):
  try:
    ax = plt.axes()
    ax.set_facecolor("navajowhite")
    if action == "s":
      df_plot = df1_plot1(df,df_Team1,set,slot,number,"s","")
      plt.scatter(df_plot["X"],df_plot["Y"],s = df_plot["COUNT"]*500,color="blue",alpha=0.5)
      df_plot = df1_plot1(df,df_Team1,set,slot,number,"s","effective")
      plt.scatter(df_plot["X"],df_plot["Y"],s=df_plot["COUNT"]*200,color="orange")
      df_plot = df1_plot1(df,df_Team1,set,slot,number,"s","p")
      plt.scatter(df_plot["X"],df_plot["Y"],s=df_plot["COUNT"]*100,color="red")
      df_plot = df1_plot1(df,df_Team1,set,slot,number,"s","m")
      ...
      return plt.show()
    if action == "a":
      df_plot = df1_plot1(df,df_Team1,set,slot,number,"a","")
      plt.scatter(df_plot["X"],df_plot["Y"],s = df_plot["COUNT"]*300,color="lightblue")
      df_plot = df1_plot1(df,df_Team1,set,slot,number,"a","effective")
      plt.scatter(df_plot["X"],df_plot["Y"],s=df_plot["COUNT"]*200,color="lightgreen")
      df_plot = df1_plot1(df,df_Team1,set,slot,number,"a","p")
      plt.scatter(df_plot["X"],df_plot["Y"],s=df_plot["COUNT"]*100,color="orange")
      df_plot = df1_plot1(df,df_Team1,set,slot,number,"a","m")
      plt.scatter(df_plot["X"],df_plot["Y"],s=df_plot["COUNT"]*70,color="white")
      df_plot = df2_plot1(df,df_Team1,set,slot,number,"a","")
      plt.scatter(df_plot["X"],df_plot["Y"],s = df_plot["COUNT"]*300,color="blue",alpha=0.5)
      df_plot = df2_plot1(df,df_Team1,set,slot,number,"a","effective")
      plt.scatter(df_plot["X"],df_plot["Y"],s=df_plot["COUNT"]*200,color="green")
      df_plot = df2_plot1(df,df_Team1,set,slot,number,"a","p")
      plt.scatter(df_plot["X"],df_plot["Y"],s=df_plot["COUNT"]*100,color="red")
      df_plot = df2_plot1(df,df_Team1,set,slot,number,"a","m")
      plt.scatter(df_plot["X"],df_plot["Y"],s=df_plot["COUNT"]*70,color="white")
      ...
      return plt.show()  
    if action == "r":
      df_plot = df1_plot1(df,df_Team1,set,slot,number,"r","a")
      plt.scatter(df_plot["X"],df_plot["Y"],s = df_plot["COUNT"]*200,color="yellow")
      df_plot = df1_plot1(df,df_Team1,set,slot,number,"r","b")
      plt.scatter(df_plot["X"],df_plot["Y"],s=df_plot["COUNT"]*200,color="orange")
      df_plot = df1_plot1(df,df_Team1,set,slot,number,"r","c")
      plt.scatter(df_plot["X"],df_plot["Y"],s=df_plot["COUNT"]*200,color="red")
      df_plot = df1_plot1(df,df_Team1,set,slot,number,"r","o")
      plt.scatter(df_plot["X"],df_plot["Y"],s=df_plot["COUNT"]*200,color="lightgreen")
      df_plot = df1_plot1(df,df_Team1,set,slot,number,"r","m")
      plt.scatter(df_plot["X"],df_plot["Y"],s=df_plot["COUNT"]*200,color="green")
...
def df1_plot1(df,df_Team1,set,slot,number,action,result):
  d1_plot = []
  ndata = {"X":0,"Y":0,"COUNT":0,}
  df1_plot1 = pd.DataFrame(ndata,index=[0])
  for k, g in df_data1(df,df_Team1,set,slot,number,action,result).groupby(["X","Y"]):
    count = len(g)
...
def df2_plot1(df,df_Team1,set,slot,number,action,result):
  d2_plot = []
  ndata = {"X":0,"Y":0,"COUNT":0,}
...
# 抽出データにx-y座標を追加
def df_data1(df,df_Team1,set,slot,number,action,result):
  df_data = dfaction1(df,df_Team1,set,slot,number,action,result).dropna(subset=["zone1"]).reset_index(drop=True)
...
# セット項目
def dfaction1(df,df_Team1,set,slot,number,action,result):
  if set =="":
    if slot =="":
      if number == "":
        if result == "":
